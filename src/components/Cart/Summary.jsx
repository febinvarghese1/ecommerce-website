...
const Summary = ({totalPrice}) => {
  ...
  const discountedTotal = Math.abs((totalPrice)*((100-10)/100)+20).toFixed(2);
  const savings = Math.abs((totalPrice-(totalPrice)*((100-10)/100)).toFixed(2));
  ...
  return (
    <div className="summary">
      ...
      <div className="summary__center">
          <div><h3>subtotal:    </h3><span> ${total}</span></div>
          <div><h3>Tax:  </h3><span> $20</span></div>
          <div><h3>Discount: </h3><span>10 %</span></div>
          <div><h3>Estimated Total:</h3> <span>{discountedTotal}</span></div>
